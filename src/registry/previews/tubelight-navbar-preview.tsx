...
import {
  TubelightNavbar,
  type TubelightNavbarItem,
} from "@/registry/ui/tubelight-navbar";
import type { PreviewProps } from "@/registry/schema";
import type { StyleSlug } from "@/lib/component-data";

/**
 * Documentation preview for the Tubelight Navbar.
 *
 * Previews live outside `src/registry/ui/` so that folder stays purely
 * distributable: what a user copies is the component, never the scaffolding used
 * to document it.
 *
 * Every `PreviewState` handled here maps to something a Tubelight Navbar
 * genuinely has: `focus` forces the ring on the active item, `disabled` dims the
 * whole bar and takes it out of the pointer path. Anything else falls through to
 * the default interactive render. The demo lets the reader click through the
 * items live and watch the tube slide — `onClickCapture` cancels the anchors'
...
function asVariant(value: string): TubelightVariant {
  return (VARIANTS.includes(value) ? value : "default") as TubelightVariant;
}

function asSize(value: string): TubelightSize {
  return (SIZES.includes(value) ? value : "md") as TubelightSize;
}

/** Copy differs per material so each recipe is shown doing real work. */
const ITEMS: Record<StyleSlug, readonly TubelightNavbarItem[]> = {
  clay: [
...
export function TubelightNavbarPreview({ material, variant, size, state }: PreviewProps) {
  ...
  return (
    <TubelightNavbar
      ...
      // Uncontrolled: start on the second item so the tube has somewhere to slide
      // from as the reader clicks around.
