...
import { PillNav, type PillNavItem } from "@/registry/ui/pill-nav";
import type { PreviewProps } from "@/registry/schema";
import type { StyleSlug } from "@/lib/component-data";

/**
 * Documentation preview for the Pill Nav.
 *
 * Previews live outside `src/registry/ui/` so that folder stays purely
 * distributable: what a user copies is the component, never the scaffolding used
 * to document it.
 *
 * Every `PreviewState` handled here maps to something a Pill Nav genuinely has:
 ...
 * watch the pill slide — `onClickCapture` cancels the anchors' navigation so the
 * page never jumps while the internal active state still updates.
 */
...
function asVariant(value: string): PillNavVariant {
  return (VARIANTS.includes(value) ? value : "default") as PillNavVariant;
}

function asSize(value: string): PillNavSize {
  return (SIZES.includes(value) ? value : "md") as PillNavSize;
}

/** Copy differs per material so each recipe is shown doing real work. */
const ITEMS: Record<StyleSlug, readonly PillNavItem[]> = {
  clay: [
...
export function PillNavPreview({ material, size, state, variant }: PreviewProps) {
